...
def _extract_read_count(metadata: str) -> list:
    init_idx = metadata.find('DP4=') + 4  # coords given are for 'D', correct.
    end_idx = metadata.find('MQ=') - 1  # coords given are for 'M', correct.
    return metadata[init_idx:end_idx].split(',')[-2:]


def compute_frequency(metadata: str) -> float:
    """
        Use depth at site and number of reads to compute frequency
        from metadata.
    """
    depth_total = int(metadata.split(';')[DEPTH_ID].split('=')[-1])
    alt_depth = 0
    for read_count in _extract_read_count(metadata):
        alt_depth += int(read_count)
    return alt_depth / depth_total * 100
...
def main(vcf_file: str, reference_file: str, genbank_file: str):
    ...
    """
    fOut = vcf_file.replace('report.vcf.gz', 'variants_context.tsv')
        ...
        with gzip.open(vcf_file, 'r') as variants:
            ...
            for record in variants:
                if record.decode()[0] != str('#'):
                    ...
                    FREQ = compute_frequency(METADATA)
